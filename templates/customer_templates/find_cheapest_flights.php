...
if ($db_conn) {
	...
	$view_query = "CREATE VIEW outbound_tickets as (SELECT t1.tID FROM Ticket t1, Comprised_Of c1 WHERE t1.tID = c1.tID AND c1.from_airport_code = '".$airport."') MINUS (SELECT t2.tID FROM Customer_Purchase cp, Discounted_Purchase dp, Ticket t2, Comprised_Of c2 WHERE t2.tID = c2.tID AND c2.from_airport_code = '".$airport."' AND cp.tID = t2.tID AND dp.tID = t2.tID)";
